				...
				while (expGenerator.hasNext()) {
					for (int i = 0; i < creativeWorksForCurrentIteration; i++) {
						if (currentTriplesCount >= triplesPerFile) {
							flushClose(fos);
							if (!silent) {
								System.out.println(Thread.currentThread().getName() + " EWorker :: Saving file #" + currentFilesCount + " with " + String.format("%,d", cwsInFileCount) + " Creative Works. Generated triples so far: " + String.format("%,d", triplesGeneratedSoFar.get()) + ". Target: " + String.format("%,d", targetTriples) + " triples");
							}
